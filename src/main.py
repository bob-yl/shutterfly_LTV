...
# Setting up the first and last Sundays to get the absolute time range in weeks
...
def TopXSimpleLTVCustomers(x, data):
    ...
    # Using the stats calculated in the digest function to calculate LTV for each customer
    for k, v in data["customers"].items():
        customer_id = k

        # ltv = average revenues per week * average visits per week * customer lifetime
        ltv_value = v["stats"]["weekly_rev_per_visits_revenue"] * v["stats"][
            "weekly_average_visits"] * customer_lifetime
...
def ingest(e, data):
    # Loading data file based on the file_path parameter existence in the event data
    raw_data = None
    if e["file_path"]:
        file_path = e["file_path"]
        raw_data = file_load(file_path)

    # Find time range in weeks
    ...
    for event in raw_data:
        ...
        # Using the Sunday date for each week as a week identifier
        ...
        if event["type"] == "CUSTOMER":
            ...
            customer_data = {"created_at": event["event_time"], "updated_at": update_date,
                             "last_name": event["last_name"],
                             "address": {"city": event["adr_city"], "state": event["adr_state"]}, "visits": [],
                             "images": {}, "orders": {}}
        ...
        # Registering orders On order update, the delta between the previous and updated revenues would be calculated
        # and marked for the current week
        if event["type"] == "ORDER":
            order_id = event["key"]

            revenues = float(event["total_amount"].strip("USD")) if event["total_amount"].strip("USD") != "" else 0
            if event["verb"] == "NEW":
                ...
                revenues_diff = None
                old_revenues = data["customers"][customer_id]["orders"][order_id]["amount"]
                ...
                revenues_diff = revenues - old_revenues
                if revenues_diff != 0:
                    if week_id not in event_count[customer_id]:
    ...
    for ck, cv in event_count.items():
        ...
        stats_data = {"weekly_rev_per_visits_revenue": total_weekly_rev / weekly_event_range,
                      "weekly_average_visits": total_visits / weekly_event_range}
